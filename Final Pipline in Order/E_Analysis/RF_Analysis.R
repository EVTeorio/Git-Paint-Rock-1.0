...
for (i in 1:10) {
  ...
  for (model_name in names(model_inputs)) {
    ...
    # Train model
    rf_model <- randomForest(SpeciesID ~ ., data = train_data, ntree = 300, importance = TRUE)
...
# Save to CSV
write.csv(summary_metrics, "E:/Thesis_Final_Data/Analysis/model_summary_metrics.csv", row.names = FALSE)
##############################################################################
...
for (sample_name in names(all_results)) {
  for (model_name in names(all_results[[sample_name]])) {
    ...
    # Confusion matrix
    cm_file <- file.path(output_dir, paste0(sample_name, "_", model_name, "_confusion.csv"))
